...
module.exports = function signupRouter (sql) {
  ...
  // ── Public signup → create 7-day trial brand ─────────────────────────────
  router.post('/signup', async (req, res) => {
    const { name, owner_name, email, phone, city, country, signup_source } = req.body || {}
    if (!name || !name.trim())            return res.status(400).json({ ok: false, error: 'Brand name is required' })
    ...
    try {
      const [existing] = await sql`SELECT id FROM brands WHERE email = ${email.toLowerCase().trim()}`
      if (existing)
      ...
      await sql`
        INSERT INTO bo_users (id, username, password, role, brand_id)
        VALUES (${boUserId}, ${boUsername}, ${boPassHash}, 'owner', ${id})
        ON CONFLICT (username) DO UPDATE SET brand_id = EXCLUDED.brand_id, role = 'owner'`
